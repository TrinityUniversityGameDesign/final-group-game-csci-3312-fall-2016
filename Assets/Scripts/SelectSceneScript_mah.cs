...
public class SelectSceneScript_mah : MonoBehaviour {
	...
	IEnumerator showCountdown(float remaining) {
		for (float r = remaining; r > 0f; r -= 0.1f) {
			countDownText.text = "Enter names now\nStarting in\n" + r.ToString ("#.0");
	...
	{
		StartCoroutine (showCountdown (delay));
