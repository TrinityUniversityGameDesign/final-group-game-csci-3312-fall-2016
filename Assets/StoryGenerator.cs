...
public class StoryGenerator : MonoBehaviour {
    ...
    */     
    ...
    string[] make_story()
    {
        ...
        MainCharacter mc = build_main_character(adjectives, occupations, names, servants, items, gods, locations);
        ...
        questions[0] = get_random(questions_opening) as string[];
        if (get_bool())
        {
            ...
            questions[1] = get_random(questions_cursed) as string[];
            if (get_bool())
            {
                // Questions below this point HAVE NOT BEEN FILLED OUT.
                string[][] questions_old = {build_question_and_answers("What adjective would you use to describe " + mc.name + "?", mc.adjective, adjectives),
                                   build_question_and_answers("What did " +  mc.he + " discover?", rel.item, items),
                                   build_question_and_answers("What adjective would you use to describe the " + rel.item + "?", rel.adjective, adjectives)};
                questions[2] = get_random(questions_old) as string[];
                story_pieces[2] = generate_old(mc, rel, god_req);
            } else
            {
                string[][] questions_horrible = {build_question_and_answers("What adjective would you use to describe " + mc.name + "?", mc.adjective, adjectives),
                                   build_question_and_answers("What did " +  mc.he + " discover?", rel.item, items),
                                   build_question_and_answers("What adjective would you use to describe the " + rel.item + "?", rel.adjective, adjectives)};
                questions[2] = get_random(questions_horrible) as string[];
        ...
        } else
        {
            ...
            string[][] questions_scorned = {build_question_and_answers("What adjective would you use to describe " + mc.name + "?", mc.adjective, adjectives),
                                   build_question_and_answers("What did " +  mc.he + " discover?", rel.item, items),
                                   build_question_and_answers("What adjective would you use to describe the " + rel.item + "?", rel.adjective, adjectives)};
            questions[1] = get_random(questions_scorned) as string[];
            if (get_bool())
            {
                story_pieces[2] = generate_killed(mc, rel, "burned alive", "fed to wild dogs", "buried alive in hot, acidic sand");
                string[][] questions_killed = {build_question_and_answers("What adjective would you use to describe " + mc.name + "?", mc.adjective, adjectives),
                                   build_question_and_answers("What did " +  mc.he + " discover?", rel.item, items),
                                   build_question_and_answers("What adjective would you use to describe the " + rel.item + "?", rel.adjective, adjectives)};
                questions[2] = get_random(questions_killed) as string[];
            }
            else
            {
                ...
                string[][] questions_stranded = {build_question_and_answers("What adjective would you use to describe " + mc.name + "?", mc.adjective, adjectives),
                                   build_question_and_answers("What did " +  mc.he + " discover?", rel.item, items),
                                   build_question_and_answers("What adjective would you use to describe the " + rel.item + "?", rel.adjective, adjectives)};
                questions[2] = get_random(questions_stranded) as string[];
            }
            ...
            string[][] questions_lost = {build_question_and_answers("What adjective would you use to describe " + mc.name + "?", mc.adjective, adjectives),
                                   build_question_and_answers("What did " +  mc.he + " discover?", rel.item, items),
                                   build_question_and_answers("What adjective would you use to describe the " + rel.item + "?", rel.adjective, adjectives)};
            questions[3] = get_random(questions_lost) as string[];
        }
        return story_pieces;
    }
